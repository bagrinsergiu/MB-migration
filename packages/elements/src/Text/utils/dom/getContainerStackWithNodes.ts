    ...
    if (_node instanceof HTMLElement) {
      ...
      if (excludeIcons) {
        ...
        if (buttons.length > 0) {
          ...
          const hasNonEmptyNodes = Array.from(_node.childNodes).some((node) => {
            if (node instanceof HTMLElement) {
              const hasButton =
                node.querySelectorAll(buttonSelector).length > 0 ||
                Array.from(buttons).includes(node);

              return !hasButton && node.textContent?.trim();
            } else {
