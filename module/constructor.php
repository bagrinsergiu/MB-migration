...
{   
    ...
    function getValueFromArrayByPath($array, $path) 
    {
        ...
        if(!is_array($array))
        {
            $this->tool->log('Bad Array', 2, $nameFunction);
            return FALSE;
        }
    ...
    public function getPageName($name = 'home')
    {
        ...
        { 
            $this->tool->log('Page does not exist in array. key: '. $name, 2, $nameFunction);
            return FALSE;
        }
    }
    ...
    public function jsonDataLoad($nameTemplate, $namePage = 'home', $path = 'default')
    {
        ...
        $jsonDataLayout = $this->tool->strReplace(Config::$pathLayoutData, array("{theme}", "{page}"), array(Config::$themes[$nameTemplate], $namePage) );

        $this->tool->log('Import: '.$jsonDataLayout,0,$nameFunction);
        ...
        {   
            $this->tool->log('File does not exist. PATH: '. $jsonDataLayout, 2, $nameFunction);
            return FALSE;
        }
    }
