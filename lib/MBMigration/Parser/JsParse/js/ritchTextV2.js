...
var allowedTags = ["P", "H1", "H2", "H3", "H4", "H5", "H6"];
...
function removeAllStylesFromHTML(node) {
    ...
    var elementsWithStyles = node.querySelectorAll(allowedTags.join(",") + "[style]");
    var elementsWithClasses = node.querySelectorAll(allowedTags.join(",") + "[class]");
...
function transformDivsToParagraphs(containerElement) {
    ...
    divElements.forEach(function (divElement) {
        ...
        for (var i = 0; i < divElement.attributes.length; i++) {
            var attr = divElement.attributes[i];
...
const toBuilderText = (data) => {
    ...
    // loop all children and attach classes
    for (let i = 0; i < node.children.length; i++) {
        const child = node.children[i];
        const classes = stylesToClasses(styles[i], families, defaultFontFamily);
