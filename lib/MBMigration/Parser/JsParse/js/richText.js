...
function removeAllVideo(node, selector) {
    const videos = node.querySelectorAll(".embedded-paste");

    videos.forEach((video, index) => {
        const tempDiv = document.createElement("div");
        tempDiv.append(video.cloneNode());

        warns[`${selector}-${index}`] = {
            type: "video",
            index: index,
...
const run = (data) => {
    ...
    node = removeAllVideo(node, data.selector);
