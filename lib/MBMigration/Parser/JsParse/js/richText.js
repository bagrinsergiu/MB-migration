...
function getParentElementOfTextNode(node) {
    ...
    return Array.from(node.childNodes).find((node) => getParentElementOfTextNode(node));
}
...
function mergeStyles(element) {
    ...
    return {...elementStyles, ...innerStyles, "line-height": elementStyles["line-height"]}
}
...
function getButtonModel(style, node) {
    ...
    return {
        ...
        colorOpacity: 1,
        text: node.text, ...(isLink && {
            linkExternal: node.href ?? "", linkType: "external", linkExternalBlank: "on",
        }),
    };
}
...
function getIconModel(style, node) {
    ...
    return {
        colorHex: rrgbToHex(style.color) ?? "#ffffff", colorOpacity: +style.opacity, ...(isLink && {
            linkExternal: parentElement.href ?? node.href ?? "",
            linkType: "external",
            linkExternalBlank: "on",
            bgColorHex: rrgbToHex(getStyle(parentElement)["background-color"]) ?? "#ffffff",
        }),
    };
}
...
function removeAllStylesFromHTML(node) {
    ...
    const elementsWithStyles = node.querySelectorAll(allowedTags.join(",") + "[style]");
    const elementsWithClasses = node.querySelectorAll(allowedTags.join(",") + "[class]");
...
function removeAllIcons(node) {
    const icons = node.querySelectorAll("[data-socialicon],[style*=\"font-family: 'Mono Social Icons Font'\"]");
    ...
    icons.forEach((icon) => {
        ...
        const model = {...getIconModel(style, icon), iconCode: iconNode.textContent.charCodeAt(0)};

        const group = iconGroups.get(parentElement) ?? {items: []};
        iconGroups.set(parentElement, {
            ...group, position, align: textAlign[style["text-align"]], items: [...group.items, model],
        });
    });
...
function extractParentElementsWithStyles(node) {
    ...
    if (shouldExtractElement(node, exceptExtractingStyle)) {
        ...
        result.push({
            uid, tagName: node.tagName, styles: mergeStyles(node),
        });
    }
...
const getTypographyStyles = (data) => {
    ...
    return allRichTextElements.map((element) => {
        ...
        return {
            ...element, styles: attributes.reduce((acc, attribute) => {
...
const run = (data) => {
    ...
    if (!node) {
        return JSON.stringify({
            error: `Element with selector ${data.selector} not found`, warns: warns,
        });
    }
    ...
    if (!node) {
        return JSON.stringify({
            error: `Element with selector ${data.selector} has no wrapper`, warns,
        });
    }
    ...
    return JSON.stringify({
        text: toBuilderText(dataText), warns: warns, embeds: embeds, icons: iconsPositions, buttons: buttonsPositions,
    });
};
