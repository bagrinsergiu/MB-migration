...
class ColorUtility
{
     ...
     */
    public function chooseLargerContrast($base, $option1, $option2): string
    {
        $this->scss = new Compiler();
        $scssCode = "
        ...
        @function luminance(\$color) {
            @return (lightness(\$color) + .05) * 100;
        }

        @function lumLight(\$color) {
            @return strip-unit((luminance(\$color) + lightness(\$color)) / 2) - 2.5;
        }
    ...
    ";
        ...
        $this->scss->setOutputStyle(OutputStyle::COMPRESSED);
        $result = $this->scss->compileString($scssCode);
        $color = $this->getColorFromRootCSS($result->getCss());
        return $color;
    }
     ...
     * @throws SassException
     */
    public function desaturateByPercent($color, int $percent): string
    {
        $scssCode = "
            @function desaturateByPercent(\$color, \$percent) {
     ...
     * @throws SassException
     */
    public function mixContrastingColor($color, $threshold = 50): string
    {
        $scssCode = "
            @function getContrastingColor(\$color, \$lightColor: #ffffff, \$darkColor: #2a2a2a) {
        ...
        $this->scss->setOutputStyle(OutputStyle::COMPRESSED);
        $result = $this->scss->compileString($scssCode);
        $colorResult = $this->getColorFromRootCSS($result->getCss());
        return $colorResult;
    }
     ...
     * @throws SassException
     */
    public function getContrastingColor($color, $lightColor = '#ffffff', $darkColor = '#2a2a2a'): string
    {
        $scssCode = "
