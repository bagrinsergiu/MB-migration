...
namespace MBMigration\Builder\Layout\Zion;
...
class Zion extends Layout
{
    ...
    {
        Utils::log('Create block menu', 1, "Zion] [createMenu");
    ...
    {
        Utils::log('Create bloc', 1, "Zion] [left_media");
    ...
    {
        Utils::log('Create bloc', 1, "Zion] [right_media");
    ...
    private function full_text(array $encoded): bool|string
    {
        ...
        if (!$this->checkArrayPath($encoded, 'settings/sections/background/filename')) {
            ...
            Utils::log('Set background', 1, "Zion] [full_text");
    ...
    {
        Utils::log('Create bloc', 1, "Zion] [full_text (parallaxScroll)");
        $decoded = $this->jsonDecode['blocks']['full-text'];
        ...
        if(!empty($encoded['settings']['sections']['background'])) {
            ...
            Utils::log('Set background', 1, "Zion] [full_text (parallaxScroll)");
            $block = json_decode($decoded['background'], true);
    ...
    {
        Utils::log('Create bloc', 1, "Zion] [left_media_circle");
    ...
    {
        Utils::log('Create bloc', 1, "Zion] [gallery_layout");
...
// brz-text-lg-left
    ...
    {
        Utils::log('Replace Title Tag: '. $html, 1, "Zion] [replaceTitleTag");
        if(empty($html))
            return '';
        $doc = new DOMDocument();
        $doc->loadHTML($html);
    ...
    private function replaceParagraphs($html, $type = ''): string {
        Utils::log('Replace Paragraph: '. $html, 1, "Zion] [replaceParagraphs");
        if(empty($html)){
            return '';
        }
