...
(() => {
  ...
  // ../../../../../../../packages/elements/src/Text/models/Image/index.ts
  function getImageModel(node) {
    ...
    return Array.from(images).map((image) => {
      ...
      const linkExternalBlank = isAnchor && parent?.getAttribute("target") === "_blank" ? "on" : "off";
  ...
  var Stack, extractInnerText, copyClassList, flattenNode, replaceWrongTags, getImageSizes, getContainerStackWithNodes;
        ...
        containerChildren.forEach((node2) => {
          if (node2 instanceof HTMLElement) {
            switch (node2.dataset.type) {
              ...
              case "image": {
                const models = getImageModel(node2);
                data.push(...models);
              }
            }
          }
