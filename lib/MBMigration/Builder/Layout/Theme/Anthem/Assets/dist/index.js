...
(() => {
  ...
  var import_fp_utilities, allowedTags, exceptExtractingStyle, defaultDesktopLineHeight, defaultTabletLineHeight, defaultMobileLineHeight, extractedAttributes, textAlign, iconSelector, buttonSelector, embedSelector, extractUrlWithoutDomain, getHref, getTarget, normalizeOpacity;
        ...
        flatNode.childNodes.forEach((node) => {
          ...
          if (_node instanceof HTMLElement) {
            ...
            if (excludeIcons) {
              ...
              if (buttons.length > 0) {
                const hasNonEmptyNodes = Array.from(_node.childNodes).some((node2) => {
                  if (node2 instanceof HTMLElement) {
                    const hasButton = node2.querySelectorAll(buttonSelector).length > 0 || Array.from(buttons).includes(node2);
                    return !hasButton && node2.textContent?.trim();
                  } else {
