...
(() => {
  ...
  var trimSpacesAtStartOfTag;
  var init_removeEmptyNodes = __esm({
    "../../../../../../../packages/elements/src/Text/utils/dom/removeEmptyNodes.ts"() {
      "use strict";
      trimSpacesAtStartOfTag = (html) => html.replace(
        /(<[^>]+>)\s{2,}([^<]*)(<\/[^>]+>)/g,
        // Match tags with two or more spaces at the start of the text content
        (_, openTag, textContent, closeTag) => `${openTag}${textContent.trimStart()}${closeTag}`
        // Trim leading spaces if no nested tags
      );
    }
  });
