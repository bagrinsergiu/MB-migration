  ...
  var init_extractAllFontFamilies = __esm({
    "../../../../../../../packages/elements/src/Dom/extractAllFontFamilies.ts"() {
      ...
      extractAllFontFamilies = () => {
        const fontFamiliesSet = /* @__PURE__ */ new Set();
        ...
        const uniqueFontFamilies = Array.from(fontFamiliesSet).flatMap((font) => font.split(",")).map(
          (font) => font.trim().replace(/^["']|["']$/g, "").replace(/\s+/g, "_").toLowerCase()
        ).filter(Boolean);
