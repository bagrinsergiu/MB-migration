...
class Boulevard extends AbstractTheme
{
    ...
    /**
     * Включает кеширование Head элемента для темы Boulevard.
     * 
     * Благодаря кешированию стили меню извлекаются только один раз для всей миграции,
     * что обеспечивает:
     * - Одинаковые стили меню для всех страниц
     * - Оптимизацию производительности (избежание повторного извлечения стилей)
     * 
     * ВАЖНО: Head НЕ сохраняется как глобальный блок (makeGlobalBlock() = false),
     * так как из-за особой структуры секций и положения меню Head должен
     * создаваться отдельно для каждой страницы. Компонент клонируется при возврате
     * из кеша, что позволяет использовать одинаковые стили, но создавать отдельный
     * блок для каждой страницы.
     * 
     * @return bool
     */
