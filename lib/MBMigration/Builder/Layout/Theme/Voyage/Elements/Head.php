    ...
    private function setParseOptions(ItemBuilder $objBlock, $options)
    {
        ...
        foreach ($result as $key => $value) {
            if($key !== 'fontFamily' || $key !== 'fontFamilyType' ||
                $key !== 'subMenuFontFamily' || $key !== 'subMenuFontFamilyType' ||
                $key !== 'fontSize' || $key !== 'subMenuFontSize' ||
                $key !== 'colorHex' || $key !== 'subMenuColorHex' ||
                $key !== 'fontWeight' || $key !== 'subMenuFontWeight')
            { continue; }
            $objBlock->item(0)->item(0)->item(1)->item(0)->item(0)->setting($key, $value);
        }
    }
}