  ...
  var require_GlobalMenu = __commonJS({
    "src/GlobalMenu/index.ts"() {
      ...
      var globalMenuExtractor = () => {
        ...
        if (span) {
          const styles2 = getNodeStyle(span);
          const bgColor = parseColorString(`${styles2["background-color"]}`);
          if (bgColor) {
            window.menuModel = {
              ...window.menuModel,
        ...
        if (subMenuItem) {
          const span2 = subMenuItem.children[0];
          const styles2 = getNodeStyle(subMenuItem);
          const color2 = parseColorString(`${styles2["color"]}`);
          if (color2) {
            window.menuModel = {
              ...window.menuModel,
              hoverSubMenuColorHex: color2.hex,
              hoverSubMenuColorOpacity: color2.opacity ?? 1,
              hoverSubMenuColorPalette: ""
            };
          }
          if (span2) {
            const styles3 = getNodeStyle(span2);
            const bgColor = parseColorString(`${styles3["background-color"]}`);
            if (bgColor) {
              window.menuModel = {
                ...window.menuModel,
