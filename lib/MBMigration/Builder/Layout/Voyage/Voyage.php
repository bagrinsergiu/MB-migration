...
namespace MBMigration\Builder\Layout\Voyage;
...
class Voyage extends Layout
{
    ...
    {
        Utils::log('Create block menu', 1, "Voyage] [createMenu");
    ...
    {
        Utils::log('Create bloc', 1, "Voyage] [left_media");
    ...
    {
        Utils::log('Create bloc', 1, "Voyage] [right_media");
    ...
    {
        Utils::log('Create full media', 1, "Voyage] [full_media");
    ...
    private function full_text(array $encoded): bool|string
    {
        ...
        if (!$this->checkArrayPath($encoded, 'settings/sections/background/filename')) {
            ...
            Utils::log('Set background', 1, "Voyage] [full_text");
    ...
    {
        Utils::log('Create bloc', 1, "Voyage] [full_text (parallaxScroll)");
        $decoded = $this->jsonDecode['blocks']['full-text'];
        ...
        if(!empty($encoded['settings']['sections']['background'])) {
            ...
            Utils::log('Set background', 1, "Voyage] [full_text (parallaxScroll)");
            $block = json_decode($decoded['background'], true);
    ...
    {
        Utils::log('Create bloc', 1, "Voyage] [left_media_circle");
    ...
    {
        Utils::log('Create bloc', 1, "Voyage] [gallery_layout");
...
// brz-text-lg-left
    ...
    {
        Utils::log('Replace Title Tag: '. $html, 1, "Voyage] [replaceTitleTag");
        if(empty($html))
            return '';
        $doc = new DOMDocument();
        $doc->loadHTML($html);
    ...
    private function replaceParagraphs($html, $type = ''): string {
        Utils::log('Replace Paragraph: '. $html, 1, "Voyage] [replaceParagraphs");
        if(empty($html)){
            return '';
        }
