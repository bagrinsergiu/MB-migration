...
class CreateMigrationMapping
{
    private array $projectUuids;
    private MySQL $DB;

    public function __construct(array $projectUuids)
    {
        try {
            Logger::initialize(
                "",
                'debug',
                './create_migration_mapping.log'
            );

            Logger::instance()->info('CreateMigrationMapping initialization started');
            $this->projectUuids = $projectUuids;

            try {
                // Note: Using same connection details as previous implementation; ideally should come from config
                $this->DB = new MySQL(
            ...
            Logger::instance()->error('Error during CreateMigrationMapping initialization: ' . $e->getMessage(), ['exception' => $e]);
        }
    }

    /**
     * Load migrated projects from migration_result_list for provided UUIDs.
     * Returns associative array [mb_project_uuid => (int)brz_project_id]
     */
    private function loadMigratedProjects(): array
    {
        if (empty($this->projectUuids)) {
            Logger::instance()->warning('No project UUIDs provided');
            return [];
        }

        try {
            $placeholders = implode(',', array_fill(0, count($this->projectUuids), '?'));
            $sql = "SELECT mb_project_uuid, brz_project_id FROM migration_result_list WHERE mb_project_uuid IN ($placeholders)";
            $rows = $this->DB->getAllRows($sql, $this->projectUuids);

            $map = [];
            foreach ($rows as $row) {
                if (empty($row['mb_project_uuid'])) { continue; }
                $brzId = (int)($row['brz_project_id'] ?? 0);
                if ($brzId <= 0) { continue; }
                $map[$row['mb_project_uuid']] = $brzId;
            }

            Logger::instance()->info('Loaded ' . count($map) . ' migrated projects from migration_result_list');
            return $map;
        } catch (\Exception $e) {
            Logger::instance()->error('Error loading migrated projects: ' . $e->getMessage(), ['exception' => $e]);
            return [];
        }
    }

    /**
     * Create mapping records in migrations_mapping for migrated projects.
     * Skips duplicates based on mb_project_uuid.
     */
    public function runMigrations(): void
    {
        try {
            $migrated = $this->loadMigratedProjects();

            $created = 0;
            $skipped = 0;

            foreach ($migrated as $uuid => $brzId) {
                $existing = $this->DB->find('SELECT id FROM migrations_mapping WHERE mb_project_uuid = ?', [$uuid]);
                ...
                try {
                    ...
                    Logger::instance()->error('Insert failed for UUID ' . $uuid . ': ' . $e->getMessage(), ['exception' => $e]);
                }
            }

            Logger::instance()->info("CreateMigrationMapping finished. Created: {$created}, Skipped (exists): {$skipped}");
        } catch (\Exception $e) {
            Logger::instance()->critical('Critical error running CreateMigrationMapping: ' . $e->getMessage(), ['exception' => $e]);
        }
    }
}
...
$createMigrationMapping = new CreateMigrationMapping($projectUuids);

$createMigrationMapping->runMigrations();
